...
export function TokamaHomepage({ locale }: Props) {
  ...
  useEffect(() => {
    const updateHeader = () => setOnHero(window.scrollY < window.innerHeight - 96);
    updateHeader();
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header} style={{ color: onHero ? "#ffffff" : "#111111" }}>
      ...
      <section className={styles.hero} aria-label="TOKAMA">
        ...
        <div className={styles.heroContent}>
          ...
          <div className={styles.heroActions}>
            <Link href={bookingHref} className={styles.lightButton}>{t.book}</Link>
            <a href="#domki" className={styles.outlineButton}>{t.explore}</a>
          </div>
        </div>
      </section>
